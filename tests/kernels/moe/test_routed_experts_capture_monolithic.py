...
from vllm.model_executor.layers.fused_moe.experts.trtllm_bf16_moe import (
    TrtLlmBf16Experts as TrtLlmBf16ExpertsMonolithic,
)
...
def test_trtllm_bf16_monolithic_supports_capture_for_all_routing() -> None:
    ...
    for routing_method in (
        ...
        assert (
            experts.supports_routing_replay_capture() is True
...
def test_routed_experts_capturer_e2e_via_monolithic_experts() -> None:
    ...
    for other_layer in range(num_layers):
        if other_layer == layer_id:
            continue
        assert (
            capturer.device_buffer[:, other_layer, :].cpu() == -1
        ).all(), f"layer {other_layer} should be untouched, got writes"
    assert (
        capturer.device_buffer[num_tokens:, layer_id, :].cpu() == -1
    ).all(), "tail rows beyond num_tokens should remain sentinel"
