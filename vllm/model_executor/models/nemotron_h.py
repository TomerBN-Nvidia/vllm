...
class NemotronHForCausalLM(
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        # Generator to filter MTP weights and route others
        def weight_generator():
            for name, weight in weights:
                # Apply mapper manually since we are bypassing AutoWeightsLoader
                if name.startswith("backbone."):
                    name = "model." + name[9:]
                name = name.replace("A_log", "A").replace("embeddings", "embed_tokens")
                ...
                if name.startswith("lm_head."):
                    ...
                    lm_head_params.add(name)
                    continue

                if name.startswith("model."):
                    # Strip "model." prefix for NemotronHModel.load_weights
                    yield name[6:], weight

        lm_head_params = set()
        # Delegate to NemotronHModel.load_weights
        model_loaded_params = self.model.load_weights(weight_generator())

        # Prefix model params back
        all_loaded_params = lm_head_params
        for name in model_loaded_params:
            all_loaded_params.add("model." + name)

        return all_loaded_params
