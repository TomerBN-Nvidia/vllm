...
class Request:
    def __init__(
        ...
        # Exact contiguous token frontier covered by materialized target and
        # draft KV. This preserves progress within a hash block.
    ...
    def truncate_block_hashes(
        ...
        self.num_materialized_eagle_tokens = min(
            self.num_materialized_eagle_tokens, num_tokens
        )
    ...
    def mark_eagle_hashes_publishable(
        ...
        self.num_materialized_eagle_tokens = max(
            self.num_materialized_eagle_tokens, acknowledged_tokens
        )
        num_hashes = min(len(self.block_hashes), acknowledged_tokens // hash_block_size)
    ...
    def invalidate_eagle_hash_publication(
        ...
        self.num_materialized_eagle_tokens = min(
            self.num_materialized_eagle_tokens, num_materialized_tokens
        )
